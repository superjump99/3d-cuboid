...
let parameters = {
    ...
    gotoCamera: function(){
        const {currentDataset, currentDatasetIdx, sequence} = labelTool;
        console.log("11")
        // window.open(`/camera-views`, '_blank');
    },
    i: -1,
...
function setHighestAvailableTrackId(label) {
    ...
    // 전체 annotationObjects.contents를 순회하여 maxTrackId 계산
    for (let fileIndex = 0; fileIndex < annotationObjects.contents.length; fileIndex++) {
        for (let i = 0; i < annotationObjects.contents[fileIndex].length; i++) {
            if (label === annotationObjects.contents[fileIndex][i]["class"]) {
                // 특정 class에 대한 현재 trackId가 maxTrackId보다 큰 경우 업데이트
                if (annotationObjects.contents[fileIndex][i]["trackId"] > maxTrackId) {
                    maxTrackId = annotationObjects.contents[fileIndex][i]["trackId"];
                }
            }
        }
    }

    // maxTrackId를 기반으로 nextTrackId 설정
    classesBoundingBox[label].nextTrackId = maxTrackId + 1;
    console.log(label, classesBoundingBox[label].nextTrackId);
    //
...
function onChangeHandler(event) {
    ...
    try{
        let objectIndexByTrackId = getObjectIndexByName(labelTool.selectedMesh.name);
        if(objectIndexByTrackId !== undefined) {
            // console.log("onChangeHandler", `${labelTool.selectedMesh.scale.x} : ${labelTool.selectedMesh.scale.y} : ${labelTool.selectedMesh.scale.z}`)
            // let cubeFolder = folderCubeArray[clickedObjectIndex];
            // const cubeWidth = cubeFolder['cubeWidth'];
            // const cubeLength = cubeFolder['cubeLength'];
            // const cubeHeight = cubeFolder['cubeHeight'];
            // cubeWidth.setValue(labelTool.selectedMesh.scale.x);
            // cubeLength.setValue(labelTool.selectedMesh.scale.y);
            // cubeHeight.setValue(labelTool.selectedMesh.scale.z);
        }

        // cubeWidth.setValue(labelTool.selectedMesh.scale.x >= 0 ? labelTool.selectedMesh.scale.x : 0);
        // cubeLength.setValue(labelTool.selectedMesh.scale.y >= 0 ? labelTool.selectedMesh.scale.y : 0);
        // cubeHeight.setValue(labelTool.selectedMesh.scale.z >= 0 ? labelTool.selectedMesh.scale.z : 0);
    }catch(e){
    ...
    if (ev.button === 0) {
        ...
        if (clickFlag === true) {
            ...
            if (labelTool.showOriginalNuScenesLabels === true && labelTool.currentDataset === labelTool.datasets.NuScenes) {
                ...
                if (annotationObjects.__selectionIndexCurrentFrame === -1) {
                    const classesO = classesBoundingBox[classesBoundingBox.getCurrentClass()];
            ...
            if (Math.abs(groundPointMouseUp.x - groundPointMouseDown.x) > 0.1) {
                ...
                let cc = classesBoundingBox.getCurrentAnnotationClassObject();
...
function init() {
    ...
    if (guiBoundingBoxMenuInitialized === false) {
        ...
        guiOptions.add(parameters, 'gotoCamera').name("Camera View");
